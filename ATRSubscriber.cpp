...
private:
    ...
    public:
        ...
        {
            SampleInfo info;
            if (reader->take_next_sample(&quaternion_, &info) == ReturnCode_t::RETCODE_OK)
            {
                if (info.valid_data)
                {
                    samples_++;
        ...
        char** argv)
{
    ...
    QuaternionSubscriber* mysub = new QuaternionSubscriber();
    if(mysub->init())
    {
